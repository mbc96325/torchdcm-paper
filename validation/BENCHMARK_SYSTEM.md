...
- `validation/datasets/open_choice_benchmark_registry.csv`
...
```bash
python validation/benchmarks/run_estimator_benchmark_suite.py --profile smoke
python validation/benchmarks/run_estimator_benchmark_suite.py --profile full
```
...
- Biogeme Swissmetro:
  ...
  - Latent Class Torch fit + Biogeme/Apollo replay.
- Biogeme Optima:
  - Ordered Logit Torch fit + Biogeme replay;
  - Ordered Probit fixed replay.
- R `mlogit`:
...
7. Run remotely on `baichuan-mo`; do not rely on local runtime results.

## Priority Queue

1. Hybrid choice:
   - Biogeme Optima hybrid choice;
   - Apollo drug choice hybrid/ordered measurement.
2. Apollo examples:
   - mode choice for MNL/NL/CNL;
   - Swiss route choice for Mixed Logit/WTP/LC.
3. R community datasets:
   - `mlogit::Electricity` for panel mixed logit;
   - `mlogit::Train` for WTP and binary choice;
   - `mlogit::Car` for high-dimensional categorical attributes.
4. Large public surveys:
