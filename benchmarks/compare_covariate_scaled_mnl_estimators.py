...
from torchdcm import Beta, ChoiceDataset, CovariateScale, CovariateScaledMultinomialLogit
...
def swissmetro_data_with_ga(df, alternatives):
...
def default_params(names: list[str]) -> dict[str, float]:
    values = {
        ...
        "B_COST": -1.2,
        "ASC_CAR": 0.6,
        "LOG_SCALE_TRAIN_GA": -0.2,
        "LOG_SCALE_CAR_GA": 0.3,
    }
    return {name: values[name] for name in names}


def run_torch_fixed(data, spec, scales, params: dict[str, float]) -> BackendResult:
    model = CovariateScaledMultinomialLogit(spec, scales)
...
def run_torch_fit(data, spec, scales, max_iter: int) -> BackendResult:
    model = CovariateScaledMultinomialLogit(spec, scales, max_iter=max_iter)
...
def write_replay_inputs(df, alternatives, params: dict[str, float], directory: Path):
    ...
    spec = {
        "model_name": f"covariate_scaled_mnl_fixed_{len(df)}",
        "alternatives": alternatives,
        "choice_col": "choice_code",
        "parameters": params,
        "scales": {
            "TRAIN": {"value": None, "terms": [{"param": "LOG_SCALE_TRAIN_GA", "variable": "GA"}]},
            "SM": {"value": 1.0, "terms": []},
            "CAR": {"value": None, "terms": [{"param": "LOG_SCALE_CAR_GA", "variable": "GA"}]},
        },
        "utility": {
            alt: {
...
def run_apollo_fixed(df, alternatives, params: dict[str, float]) -> BackendResult:
    ...
    script = Path(__file__).resolve().parent / "apollo" / "R" / "run_covariate_scaled_mnl_fixed.R"
    if not script.exists():
        return BackendResult(backend="apollo_r_fixed", available=False, message=f"Missing script: {script}")
    with tempfile.TemporaryDirectory(prefix="torchdcm_apollo_cov_scaled_") as tmp:
        tmp_path = Path(tmp)
...
def run_biogeme_fixed(df, alternatives, params: dict[str, float]) -> BackendResult:
    ...
    try:
        database = db.Database("torchdcm_covariate_scaled_mnl_fixed", wide_df.drop(columns=["choice"]))
        choice = Variable("choice_code")
        av = {code_by_alt[alt]: Variable(f"avail_{alt.lower()}") for alt in alternatives}
        scale = {
            "TRAIN": exp(params["LOG_SCALE_TRAIN_GA"] * Variable("GA")),
            "SM": 1.0,
            "CAR": exp(params["LOG_SCALE_CAR_GA"] * Variable("GA")),
        }
        utility = {}
        for alt in alternatives:
            ...
            utility[code_by_alt[alt]] = raw / scale[alt]
        ...
        biogeme.model_name = "torchdcm_covariate_scaled_mnl_fixed"
...
def print_results(results: list[BackendResult], reference: str, n_obs: int, mode: str):
    ...
    print("case: biogeme_swissmetro_covariate_scaled_mnl")
    print(f"mode: {mode}")
    print(f"n_obs: {n_obs}")
    print("alignment:")
    if mode == "fixed":
    ...
    print("  model: Covariate-scaled MNL with exp(log-scale) alternative scales")
    print("  scale_covariate: GA for TRAIN and CAR; SM fixed to 1")
...
def main() -> None:
    ...
    df, _, base_spec, alternatives = load_biogeme_swissmetro(args.n_obs)
    data = swissmetro_data_with_ga(df, alternatives)
    ...
    names = [*base_spec.parameter_names, "LOG_SCALE_TRAIN_GA", "LOG_SCALE_CAR_GA"]
