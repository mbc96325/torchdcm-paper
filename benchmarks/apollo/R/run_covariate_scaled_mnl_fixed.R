...
for (j in seq_along(alternatives)) {
  ...
  raw <- rep(0, n_obs)
  if (!is.null(item$asc)) {
    raw <- raw + params[[item$asc]]
  }
  raw <- raw + params[["B_TIME"]] * database[[item$time]]
  raw <- raw + params[["B_COST"]] * database[[item$cost]]
  log_scale <- rep(0, n_obs)
  scale_spec <- spec$scales[[alt]]
  if (!is.null(scale_spec$value)) {
    log_scale <- log(scale_spec$value)
  }
  for (term in scale_spec$terms) {
    variable_values <- rep(1, n_obs)
    if (!is.null(term$variable)) {
      variable_values <- database[[term$variable]]
    }
    log_scale <- log_scale + params[[term$param]] * variable_values
  }
  utilities[, j] <- raw / exp(log_scale)
