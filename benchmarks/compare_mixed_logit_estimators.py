...
def result_payload(case: str, n_obs: int, n_draws: int, panel: bool, mode: str, rows: dict[str, dict]) -> dict:
    return {
        ...
        "panel": panel,
        "mode": mode,
        "consistent": is_full_consistent(rows, n_obs) if mode == "full-estimation" else True,
        "consistency_rule": (
            "Relaxed full-estimation consistency against Biogeme shared-draw result: "
            "|LL diff| <= max(0.25, 1e-5*|LL|, 0.01*N), max parameter diff <= 0.05, "
