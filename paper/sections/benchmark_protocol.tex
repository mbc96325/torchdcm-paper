...
Estimator comparisons use the same compute device whenever the compared packages expose a meaningful device choice.
Because Biogeme, Apollo, \texttt{mlogit}, and \texttt{gmnl} run on CPU in the current benchmark harness, the paper-facing cross-estimator runtime tables run TorchDCM on CPU as well.
TorchDCM CUDA runs are retained only as separate Torch-only scaling profiles, not as cross-estimator runtime entries.
