...
\input{tables/generated_mnl}

Table~\ref{tab:generated_mnl} reports the synthetic MNL full-estimation comparisons.
...
\input{tables/generated_nl}

Table~\ref{tab:generated_nl} reports the synthetic nested-logit comparisons.
The five rows vary baseline scale, larger $N$, larger $J$, larger $K$, and high feature correlation $\rho$ under generated two-nest specifications.
As in the real-data NL table, consistency is evaluated against Biogeme; Apollo is included as an additional runtime and final-objective diagnostic.
...
\input{tables/generated_mixl}

Table~\ref{tab:generated_mixl} reports the synthetic mixed-logit comparisons.
The five rows use 2--4 independent normal random coefficients with 32 shared antithetic simulation draws while varying $N$, $J$, $K$, and $\rho$.
As in the real-data MixL table, consistency is evaluated against Biogeme; Apollo is included as an additional runtime and final-objective diagnostic.
All reported synthetic mixed-logit rows are consistent with the Biogeme reference under the prespecified tolerances.

\input{tables/generated_stress}
...
\input{tables/torch_device_stress}
