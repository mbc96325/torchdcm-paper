...
The remaining completed rows are retained as diagnostics rather than forced into agreement: both estimators solve the same simulated-likelihood specification, but the nonconvex mixed-logit objective, package-specific optimizer behavior, or non-finite covariance/probability values lead to different full-estimation solutions.
The generated validation artifacts record the exact utility specification, random-coefficient set, likelihood differences, parameter differences, and probability differences for each dataset.
